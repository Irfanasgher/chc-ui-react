...
function* getHobbiystProducts({ payload }) {
    try {
        if (isStaticData === false) {
            const data = yield call(StaticProductRepository.getHobbiystPorducts, payload);
            yield put(getHobbiystProductsSuccess(data));
        } else {
            const data = yield call(StaticProductRepository.getHobbiystPorducts, payload);
            yield put(getHobbiystProductsSuccess(data));
        }
...
export default function* rootSaga() {
    ...
    yield all([takeEvery(actionTypes.GET_HOBBIYST_PRODUCTS, getHobbiystProducts)]);
    yield all([
